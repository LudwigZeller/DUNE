...
__ARTE_INIT_ int main(int argc, char **argv)
{
    ...
    if(argc > 1)
    {
        ...
        if(!mat.empty())
        {
            //!! De-alloc original Camera Provider
            //camera_provider->stop();
            //delete camera_provider;

            //!! Discrete -> millimeter conversion
        ...
        {
            std::cout << ".res -> Matrix loading error! Please check the input file!" << std::endl;
            return -1;
        }
    }
    ...
    Filter::AssetPlacementWorker resource_placement{"Resource_Placement_Worker", BC_Trees, tree_asmak_WIDTH, tree_asmak_HEIGHT, tree_asmak_DATA};
    ...
    //remove in thing CalibWorker calib_worker{"Calib_Worker"};
    //remove in thing Simulation::GameLogicWorker game_logic_worker{"Game_Logic_Worker"};
    //remove in thing Simulation::GameDrawWorker game_draw_worker{"Game_Draw_Worker", &game_logic_worker};
    ...
    //remove pipeline_smooth.push_worker(&game_logic_worker);
    pipeline_smooth.push_worker(&colorize_worker);
    pipeline_smooth.push_worker(&interpolator_worker);
    //remove pipeline_smooth.push_worker(&game_draw_worker);
    ...
    pipeline_blockcraft.push_worker(&resource_placement);

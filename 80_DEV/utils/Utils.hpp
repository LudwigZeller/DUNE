...
inline void dres_save_color_mat(const cv::Mat &cln, const std::string name, const std::string path)
{
...
inline void dres_save_depth_mat(const cv::Mat &cln, const std::string name, const std::string path)
{
...
inline void mat_meta_translate(cv::Mat &m, cv::Point2i translate)
{
...
#define macro_depthAssetToMatrix(matrix, width, height, data) { \
    short cursed[] = data;              \
    matrix = dres_asset_to_depth_mat(width, height, cursed); \
}
...
inline void dres_save_depth_mat(std::string path, cv::Mat mat)
{
...
inline cv::Mat res_load_depth_mat(const std::string &path)
{
    if(!file_exists(path))
    {
        return {};
